...
class ReduceTest < Minitest::Test
  ...
  def test_capitalize_keywords_in_phrase_one_fish_two_fish_red_fish_blue_fish
    skip
    keywords = ["fish", "blue"]
    result = keywords.reduce('one fish two fish red fish blue fish') do |val, word|
      word.capitalize if val == keywords
    end
    # Your code goes here
  ...
  def test_subtract_smallest_values_from_100
    ...
    result = elements.reduce(100) do |val, i|
      val - i.min
    end
    # Your code goes here
  ...
  def test_add_all_the_second_values_together
    ...
    result = elements.reduce(0) do |sum, i|
      sum + i[1]
    end
