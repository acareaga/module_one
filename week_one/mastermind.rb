...
def play_game # main sequence for game play
  ...
  loop do
    ...
    @user_guess = gets.chomp.downcase.chars
    if @user_guess.chars == @mystery_sequence
      ...
      @user_guess.each_with_index do |color, index|
        if color == @mystery_sequence[color]
          colors_correct += 1
        else index == @mystery_sequence[index].to_i
          positions_correct += 1
        end
      end
...
def winning_sequence_play_again_or_quit
  ...
  seconds_played = @start_time.sec + (@end_time.sec - @start_time.sec) # need to fix +/- sec on min change
